    ...
    <div className="bg-[#050505] text-[#F5F5F5] font-sans antialiased selection:bg-[#EA5211] selection:text-white min-h-screen pt-24">
      ...
      <main className="max-w-6xl mx-auto px-6 sm:px-12 py-12 space-y-16">
        ...
        <div className="flex items-center justify-between border-b border-white/10 pb-8">
          <button
            onClick={() => navigate(-1)}
            className="group flex items-center gap-2 font-sans text-xs font-medium text-neutral-400 hover:text-white uppercase tracking-[0.2em] transition-colors cursor-pointer"
          >
            <span className="group-hover:-translate-x-1 transition-transform">←</span>
            <span>Back to Work</span>
          </button>
          
          <div className="flex items-center gap-3 font-sans text-xs font-medium text-neutral-400 uppercase tracking-[0.2em]">
        ...
        <motion.div 
          ...
          className="space-y-6"
        >
          <div className="flex flex-wrap items-center gap-3 font-sans text-xs font-medium">
          ...
          {/* Quick Info Grid */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-8 font-sans text-xs">
        ...
        {project.coverImage ? (
          ...
          <div className={`w-full h-96 rounded-3xl border border-white/10 bg-gradient-to-br ${project.imageBg || 'from-neutral-900 to-black'} p-12 flex flex-col justify-center items-center text-center space-y-4 shadow-2xl relative overflow-hidden`}>
            <div className="w-20 h-20 rounded-2xl bg-white/10 border border-white/20 flex items-center justify-center text-2xl font-bold font-sans text-[#EA5211] shadow-xl">
              {project.id}
            </div>
            <h3 className="text-3xl font-light text-white">{project.name} Digital Prototype</h3>
            <p className="text-xs font-sans text-neutral-400 uppercase tracking-[0.2em] font-medium max-w-md">
        ...
        <div className="space-y-16 pt-8 border-t border-white/10">
          ...
          {cs.overview && (
            <section className="space-y-4">
              <span className="font-sans text-xs text-[#EA5211] uppercase tracking-[0.2em] block font-semibold">
          ...
          {cs.problem && (
            <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="p-8 rounded-3xl border border-red-500/20 bg-red-500/[0.02] space-y-4">
                <span className="font-sans text-xs text-red-400 uppercase tracking-[0.2em] block font-semibold">
                  THE PROBLEM
                </span>
              ...
              {cs.finalOutcome && (
                <div className="p-8 rounded-3xl border border-emerald-500/20 bg-emerald-500/[0.02] space-y-4">
                  <span className="font-sans text-xs text-emerald-400 uppercase tracking-[0.2em] block font-semibold">
                    THE OUTCOME
                  </span>
                  <p className="text-neutral-300 text-sm sm:text-base leading-relaxed font-light">
                    {cs.finalOutcome}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
                {cs.insights.map((insight, idx) => (
                  <div key={idx} className="p-5 rounded-2xl border border-white/10 bg-black/40 space-y-2">
                    <span className="font-sans text-xs text-[#EA5211] font-bold">0{idx + 1}</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {cs.userJourney && (
                <div className="p-6 rounded-2xl border border-white/10 bg-white/[0.02] space-y-2">
                  <h4 className="font-sans text-xs text-neutral-400 uppercase tracking-[0.2em] font-medium">User Journey</h4>
                  <p className="text-sm text-neutral-200 font-light leading-relaxed">{cs.userJourney}</p>
                </div>
              )}
              {cs.visualDesign && (
                <div className="p-6 rounded-2xl border border-white/10 bg-white/[0.02] space-y-2">
                  <h4 className="font-sans text-xs text-neutral-400 uppercase tracking-[0.2em] font-medium">Visual Design System</h4>
                  <p className="text-sm text-neutral-200 font-light leading-relaxed">{cs.visualDesign}</p>
                </div>
              )}
              {cs.prototype && (
                <div className="p-6 rounded-2xl border border-white/10 bg-white/[0.02] space-y-2">
                  <h4 className="font-sans text-xs text-neutral-400 uppercase tracking-[0.2em] font-medium">Prototyping</h4>
                  <p className="text-sm text-neutral-200 font-light leading-relaxed">{cs.prototype}</p>
                </div>
              )}
              {cs.reflection && (
                <div className="p-6 rounded-2xl border border-white/10 bg-white/[0.02] space-y-2">
                  <h4 className="font-sans text-xs text-neutral-400 uppercase tracking-[0.2em] font-medium">Reflection</h4>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 pt-16 border-t border-white/10">
          ...
          >
            <span className="font-sans text-xs font-medium text-neutral-400 uppercase tracking-[0.2em] group-hover:text-white transition-colors">
              ← Previous Project
            </span>
            <div>
              <span className="font-sans text-xs text-[#EA5211] font-medium block mb-1">{prevProject.category}</span>
          ...
          >
            ...
            <div>
              <span className="font-sans text-xs text-[#EA5211] font-medium block mb-1">{nextProject.category}</span>
