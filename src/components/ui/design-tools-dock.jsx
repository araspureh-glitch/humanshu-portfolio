...
// --- 8 Tools Matching Photorealistic Render Specs ---
const toolsData = [
  {
    id: "figma",
    name: "Figma",
  ...
  },
  {
    ...
    borderColor: "#EC4899",
    icon: (isHovered) => (
      <svg viewBox="0 0 24 24" className="w-5 h-5 sm:w-6 sm:h-6 transition-all duration-300" fill={isHovered ? "url(#agGradClean)" : "#FFFFFF"}>
        <path d="M12 3L2 19h4.5l2.2-4h6.6l2.2 4H22L12 3zm0 4.5l2.3 4.2H9.7L12 7.5z" />
        <defs>
          <linearGradient id="agGradClean" x1="0%" y1="0%" x2="100%" y2="100%">
...
export function DesignToolsDock() {
  const [hoveredIndex, setHoveredIndex] = useState(0); // Figma active & glowing by default
  ...
  return (
    <section className="w-full bg-black text-[#F5F5F5] py-20 px-4 sm:px-8 flex flex-col items-center justify-center relative z-20 overflow-hidden select-none">
      ...
      <div className="w-full max-w-5xl flex flex-col items-start space-y-4">
        ...
        {/* Near-Eye-Level 3/4 Side View Perspective Stage */}
        <div 
          className="w-full flex justify-center py-8 overflow-visible"
          style={{
            perspective: "1600px",
            perspectiveOrigin: "25% 40%", // Low near-eye-level 3/4 side angle
          }}
        >
          
          {/* Macro Keypad Device Body (Sleek Dark Charcoal/Black Glass Enclosure) */}
          <div 
              ...
              {toolsData.map((tool, index) => {
                ...
                return (
                  <div key={tool.id} className="relative flex flex-col items-center" style={{ transformStyle: "preserve-3d" }}>
                    ...
                    {/* Glossy Translucent Frosted Glass 3D Physical Key */}
                    <motion.button
