...
interface InteractiveHoverLinksProps {
  links?: typeof PORTFOLIO_PROJECT_LINKS;
  onProjectSelect?: (slug: string) => void;
}

export function InteractiveHoverLinks({
  links = PORTFOLIO_PROJECT_LINKS.slice(0, 3),
  onProjectSelect,
}: InteractiveHoverLinksProps) {
  return (
    <section className="bg-[#050505] text-[#F5F5F5] py-12 md:py-20 px-4 md:px-8 w-full border-t border-b border-white/10 relative">
      <div className="mx-auto max-w-6xl">
        ...
        {links.map((link) => (
          <LinkItem key={link.heading} {...link} onProjectSelect={onProjectSelect} />
        ))}
...
function LinkItem({ heading, imgSrc, subheading, href, onProjectSelect }: LinkProps) {
