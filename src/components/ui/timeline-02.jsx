...
import { ChevronDown, ChevronUp, Calendar, MapPin, Briefcase, Sparkles, CheckCircle2 } from "lucide-react";
...
function TimelineCard({ entry, index, isExpanded, toggleExpand }) {
  const cardRef = useRef(null);
  
  // Track individual card progress relative to viewport center
  const { scrollYProgress } = useScroll({
    target: cardRef,
    offset: ["start 85%", "center center", "end 15%"]
  });

  // Map scroll progress to scale, glow opacity and accent highlight
  ...
  return (
    ...
    >
      {/* Animated Glowing Node Dot along the timeline beam */}
      <motion.div 
      ...
      {/* Release Timeline Card Container */}
      <motion.div 
      ...
      >
        {/* Subtle Ambient Hover Glow in Background */}
        <motion.div 
        ...
        {/* Accordion / Expandable Key Deliverables & Highlights */}
        <AnimatePresence>
...
export default function Timeline_02({ data = defaultTimelineData, title = "Work Experience" }) {
  // First item expanded by default for immediate engagement
  const [expandedItems, setExpandedItems] = useState({ 0: true });
  const containerRef = useRef(null);

  // Scroll driven progress beam along the vertical timeline line
  const { scrollYProgress } = useScroll({
    target: containerRef,
  ...
  return (
    <section className="bg-[#050505] text-[#F5F5F5] py-16 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-5xl mx-auto">
        ...
        {/* Vertical Release Timeline Container */}
        <div ref={containerRef} className="relative space-y-12 sm:space-y-16">
          ...
          {/* Animated Scroll-Driven Glowing Beam Line */}
          <motion.div
